...
- Suggested transfers section uses the longest window to propose moving available cash from surplus accounts to those projected to go negative, aiming to cover their drop dates. Surpluses are shared across accounts with the same earliest drop date so one account does not consume all available cash. Partial/insufficient coverage is not suggested; uncovered drops are listed separately.
